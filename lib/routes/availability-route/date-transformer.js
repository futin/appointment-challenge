 ...
 * @returns {*}
 */
const compileTimeIntervalSums = listOfTimes => listOfTimes.map(({ begin, end }) => timeDifferenceInMinutes(begin, end));

// validates the interval of times, and returns true if there is
// at least one interval that satisfies the requested condition
const intervalValidation = consultationDuration => intervals =>
  intervals.some(interval => interval >= consultationDuration);

/**
 * Based on provided begin/end date, build an array of dates with custom attributes. It uses "allDay" logic, where
 * days in-between are considered as "whole day availability" check.
 *
...
/**
 * Based on provided begin/end date, build an array of dates with custom attributes. It does not use "allDay" logic,
 * and each day has the same beginHm - endHm time limit.
 *
 *    {String}  beginHm          - start of the requested availability time, represented in HH:mm
 *    {String}  endHm            - end of the requested availability time, represented in HH:mm
 *    {String}  dateWithoutTime  - as the name says... date with no time, represented as YYYY-MM-DD
