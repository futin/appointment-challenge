...
const { updateResourceTimesLeft } = require('./data-transformer');
    ...
    _.forEach(rooms, (room) => {
      ...
      _.forEach(doctors, (doctor) => {
        ...
        if (doctorConsultationTL === 'booked' || roomConsultationTL === 'booked') {
          logger(' Doctor/Room %s/%s is unavailable for further booking. skipping availability',
            doctor.name, room.name);
          return;
        }

        if (!roomConsultationTL) {
          // init "timesLeft" if it is the first time we are using this room
          updateResourceTimesLeft(room, dateWithoutTime, dayOfWeek);
        }

        if (!doctorConsultationTL) {
          // init "timesLeft" if it is the first time we are using this doctor
          updateResourceTimesLeft(doctor, dateWithoutTime, dayOfWeek);
        }
        ...
        const availabilityList = compareUtil
          .compileValidTimeInterval(doctor, room, dateConfig, consultDuration, showIds);
