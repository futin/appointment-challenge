  ...
  return rooms.reduce((totalResults, room, i) => {
    ...
    doctors.forEach((doctor, j) => {
      ...
      dateConfigs.forEach((dateConfig) => {
        ...
        const validTimeInterval = compareUtil
          .compileValidTimeInterval(room.times[dayOfWeek], doctor.times[dayOfWeek]);
        ...
        const timeIntervalSum = dateTransformer.compileTimeIntervalSum(validTimeInterval);

        if (!doctorConsultationTL) {
          const doctorsTimeIntervalInitSum = dateTransformer.compileTimeIntervalSum(doctor.times[dayOfWeek]);

          // init "timesLeft" if it is the first time we are using this doctor
          Object.assign(doctor, {
            timesLeft: {
              [dateWithoutTime]: [doctorsTimeIntervalInitSum],
            },
          });
        }
        ...
        if (!(timeIntervalSum < consultDuration && atLeastOneIntervalAvailable)) {
          logger(` Doctor %s is unavailable for further booking.
                   His time interval: %o and requested consultation duration: %s`,
          doctor.name, updatedDoctorsTL.join(','), consultDuration);
