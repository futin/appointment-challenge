  ...
  return _.reduce(rooms, (totalResults, room, i) => {
    ...
    _.forEach(doctors, (doctor, j) => {
      ...
      dateConfigs.forEach((dateConfig) => {
        ...
        const atLeastOneIntervalAvailable = dateTransformer
          .intervalValidation(consultDuration)(validCombinedTimeIntervals);
        ...
        if (!atLeastOneIntervalAvailable) {
          ...
          doctor.name, room.name, validCombinedTimeIntervals.join(','), consultDuration);
          return;
        }
      });
    });
