...
const updateResource = (resource, dates) => {
  ...
  const updatedResources = resourceTimesLeft.map((resourceTimeLeft) => {
    ...
    if (moment(beginConsultation).isAfter(moment(end)) || foundIt) {
...
const updateTimesLeftByConsultations = ([doctors, rooms, consultations]) => {
  ...
  return consultations.reduce((result, consultation) => {
    ...
    // compile date without time
    const dates = {
      dateWithoutTime: moment.utc(begin).format('YYYY-MM-DD'),
      beginTime: moment.utc(begin).format('HH:mm'),
      endTime: moment.utc(end).format('HH:mm'),
      dayOfWeek: moment.utc(begin).day(),
    };
